...
from ( 
	...
	where objid=$P{applicationid} and state in ('COMPLETED','RELEASE')
	union 
	select objid as appid from business_application 
	where parentapplicationid=$P{applicationid} and state in ('COMPLETED','RELEASE')
)xx 
...
where p.objid = $P{permitid}  
	and a.state in ('COMPLETED','RELEASE') 
